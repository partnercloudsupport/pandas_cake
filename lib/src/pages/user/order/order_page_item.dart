...
class OrderPageItem extends StatelessWidget {
  OrderPageItem({this.document, this.onSaveItem, this.active, this.index});
  ...
  Widget build(BuildContext context) {
    ...
    return Hero(
      ...
      child: GestureDetector(
        onTap: () => _settingModalBottomSheet(context, item, index),
  ...
  Future _settingModalBottomSheet(context, Item item, String index) async {
    Order order = await Navigator.of(context).push(
      new MaterialPageRoute<Order>(
        builder: (context) {
          return BlocProvider<NewOrderBloc>(
            child: NewOrderPage(),
            bloc: NewOrderBloc(item: item, tag: index),
          );
        },
      ),
    );
